...
* [Course Materials](https://storage.googleapis.com/bah-public/presentations/web-apps.pdf)

## Exercise 1: Recon with `gobuster`

`Gobuster` is a brute-force tool, written in Go and a more modern alternative to `Dirbuster`. It can brute force:
...
Tools like `gobuster` use "wordlists", or a file that contains many (many!) words that are used to find directories, files, crack passwords, etc. In fact, you should have used a wordlist in the Wireless module to crack a WPA2 password.
...
4. In Burp Suite, find the login request you just sent (it will look like `GET /dvwa/vulnerabilities/brute?userna...`). Right click -> click "Send to Intruder".
...
10. At the top, click `Start Attack`. If you used `john.lst`, you should see pretty quickly that one of the passwords gave a result that was flagged for the string "Welcome" (hint: you already logged in with the admin password earlier...)
...
* **\<star-wars-reference\> Use the source Luke! * \</star-wars-reference\>**
* Hint: If `ping` returns an error, you won't see anything.
* What user is PHP running as?

### Task 6.2

Spawn a reverse shell using Netcat. Here is a [reverse shell cheat sheet](http://pentestmonkey.net/cheat-sheet/shells/reverse-shell-cheat-sheet).

Next, start a listener using netcat from Kali.

```
nc -v -l -p 4444
```

Next, inject a command that calls back to the netcat listener, and gives you a shell.
