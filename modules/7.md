...
Metasploit uses a `postgresql` database to store results. Run the following to initialize the database:

```
$ msfdb init
```

Next, open `msfconsole` and run `db_status`. You should see:
...
From `msfconsole`, using tab completion, find out what services Metasploit can emulate from: `use auxiliary/server/` and `use auxiliary/server/capture/`
...
* Set up an FTP password capture session using Metasploit. Log into this session from Windows Command Prompt with `ftp <kali ip>`
* Observe the `msfconsole` window while doing this. What do you see?

### Task 1.4

In `msfconsole` kill the FTP background job. `jobs -h` should help.
...
Metasploitable2 has accounts with easily guessable passwords. Enumerate the accounts and guess the passwords using Metasploit. Wordlists can be found in `/usr/share/wordlists`. Make sure `db_status` shows the following:
...
View the captured credentials in the database. What is the password for `sys` account?

### Bonus

Using the results from your active recon and information about services in the database, figure out an alternate way to log into `metasploitable` not involving Samba.

## Exercise 3: Crack An NTLM-hashed Password

### Task 3.1

Using the **MS08-067** vulnerability, exploit and setup a [Meterpreter](http://www.offensive-security.com/metasploit-unleashed/Meterpreter_Basics) session with **windows**
...
Display the hashes out of [SAM](http://en.wikipedia.org/wiki/Security_Accounts_Manager). Suspend the Meterpreter session to the background.

* You can display your current background sessions with the `session` command.
...
Dump the hashes out of SAM with the `post/windows/gather/smart_hashdump` utility
...
KoreLogic throws a yearly contest held at DefCon called [Crack Me If You Can](http://contest-2011.korelogic.com/). Your VM has the contest from 2011.
