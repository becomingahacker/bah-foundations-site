...
Using the discovered SNMPv2 community string and the [Cisco-Config-Copy-MIB](http://tools.cisco.com/Support/SNMP/do/BrowseMIB.do?local=en&mibName=CISCO-CONFIG-COPY-MIB), make the router upload its configuration via TFTP to **kali.pod.becomingahacker.com**. The uploaded configuration should be placed in `/srv/tftp`. A tool using [Scapy](http://www.secdev.org/projects/scapy/doc/) is provided to help you with this. ( `~/becoming-a-hacker/networking/get-cisco-cfg.py` ) An access control list (ACL) configured on the router is blocking you--**bypass it!** Verify the TFTP server is running:
